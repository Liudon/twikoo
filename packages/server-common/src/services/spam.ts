 ...
 *
 * 使用固定问题判断整条评论是否为垃圾内容；state 同时包含正文、昵称和网址，
...
async function checkByJev(
  ...
  const score = data?.answers?.spam?.noul;
  ...
  logger.info(
    `Jev 判定为 ${isSpam ? "SPAM" : "HAM"} (score=${score.toFixed(4)}, threshold=${threshold}, model="${data.model || model}")`,
  );
  return isSpam;
}
